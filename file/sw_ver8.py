	...
	for DF_element in  df_DP['DP'].values.tolist():
		...
		while(len(temp_df.loc[temp_df['stopby']==0]) >0):
			...
			if current_point =='DP':
				...
				Path_length_list.append(Max_distance_row['DP_운행거리'])
				...
				Path_length_list.append(lowest_degree_row['운행거리'])
				next_check_time = 60*(temp_df.loc[temp_df['code']==current_point]['DP_운행거리'].values.tolist()[0]/temp_df.loc[temp_df['code']==current_point]['평균 운행 속도(km/h)'].values.tolist()[0])
			if  Rest_time_list[-1]- next_check_time>0 and Rest_Stock_list[-1] >0:
				...
				current_point ='DP'
				Point_list.append(current_point)
				Path_length_list.append(temp_df.loc[temp_df['code']==current_point]['DP_운행거리'].values.tolist()[0])
				...
				Path_length_list.append(temp_df.loc[temp_df['code']==current_point]['DP_운행거리'].values.tolist()[0])
				length_list.append(Path_length_list)
		...
		print(pd.DataFrame(length_list))
		total_list =[]
		counting_list = []
		No_truck =1
		...
		DF.to_csv("./t_result/test_"+DF_element+"_"+day+".csv",index=False ,encoding='utf-8-sig')
		print(DF)
