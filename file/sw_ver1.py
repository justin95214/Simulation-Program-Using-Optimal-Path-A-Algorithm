	...
	for DF_element in  df_DP['DP'].values.tolist():
		...
		while(True):
			...
			if time - drive_time > 0 and  Rest_Stock_list[-1]>0 :
				...
				print("degree : ",lowest_degree_row['각도'])
				temp_df = temp_df.drop(temp_df.loc[(temp_df['각도'] == lowest_degree_row['각도'])].index)
				#temp_df.loc[(temp_df['각도']==lowest_degree_row['각도']),'stopby']=1
				#change the current location
				...
				# lowest degree
				lowest_degree_row = again_temp_df.iloc[0]
				temp_df.drop(temp_df.loc[(temp_df['각도'] == lowest_degree_row['각도'])].index)
				#temp_df = temp_df.loc[(temp_df['각도'] == lowest_degree_row['각도']), 'stopby'] = 1
				# change the current location
				current_point = lowest_degree_row['code']
				# Point order list
				Point_list.append(lowest_degree_row['code'])
