...
import io
# df-client info
df_client = pd.read_excel('./new_case/PR_week.xlsx')
#print(df_client.head(10))

# DP address
df_DP = pd.read_excel('./new_case/DP_raw.xlsx')
#print(df_DP.head(5))
df_client = pd.merge(left=df_client, right=df_DP, on="DP", how="right")
df_client = df_client
# DF truck & time info df
df_info = pd.read_excel('./new_case/sheet.xlsx')
# merge the info
...
for day in week_num:
	...
	for DF_element in  df_DP['DP'].values.tolist():
		...
		print(temp_df.iloc[0]['대당 운행시간(분)'])
		...
		while(len(temp_df.loc[temp_df['stopby']==0]) >0):
			...
			if current_point =='DP':
				...
				temp_df['DP_y곡률값']=temp_df['Latitude'].astype('float')*3600
				temp_df['DP_x곡률값']=temp_df['Longitude'].astype('float')*3600
				...
				temp_df['DP_경도거리']= abs(temp_df['y곡률값']-temp_df['DP_y곡률값'])*0.0306
			...
			if  Rest_time_list[-1]- next_check_time>0 and Rest_Stock_list[-1] >0  and up_time<360:
				...
				if up_time < 360 and len(temp_df.loc[temp_df['stopby']==0]) >1:
