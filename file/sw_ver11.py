	...
	for DF_element in  df_DP['DP'].values.tolist():
		...
		while(len(temp_df.loc[temp_df['stopby']==0]) >0):
			...
			if  Rest_time_list[-1]- next_check_time>0 and Rest_Stock_list[-1] >0  and up_time<360:
				...
				print("Left  0 counti :",len(temp_df.loc[temp_df['stopby']==0]),"zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz")
				if up_time < 360 or len(temp_df.loc[temp_df['stopby']==0]) >2:
