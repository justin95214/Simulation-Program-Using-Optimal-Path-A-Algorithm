...
total_distance, day, get_up, get_off, distance_list, time_list)
		...
		if route_check ==True:
			...
			only_route.append(trace_list)
			only_time.append([str(float(time_list[0]) - float(time_list[-1])), float(time_list[-1])])
...
left_drive_time, total_distance,day, get_up, get_off, distance_list, time_list)
	...
	tt.autolog()
	return temp_df, route_list, total_distance, only_route, only_time
...
for day in week_num:
	for DF_element in  df_DP['DP'].values.tolist()[15:]:
		...
		temp_df, route_list, total_distance, only_route, only_time = route(temp_df, DF_element, day, current_point)
		...
		for i in range(len(only_route)):
			for _ in range(max_col-len(only_route[i])):
				only_route[i].append(" ")

		route_df = pd.DataFrame(only_route)
		time_df = pd.DataFrame(only_time, columns=["소요시간","남는 시간"])
		index_list = [ 'R'+str(i) for i in range(1,len(only_route)+1)]
		index_df = pd.DataFrame(index_list, columns =["index"])
		
		concat0_df = pd.concat([index_df,route_df],axis =1)
		concat1_df = pd.concat([index_df,time_df],axis =1)

		concat0_df = concat0_df.set_index('index')
		concat1_df = concat1_df.set_index('index')

		writer = pd.ExcelWriter("./test1/"+day+"_"+DF_element+'test_after.xlsx', engine = 'xlsxwriter')

		concat0_df.to_excel(writer, sheet_name = '경로')
		concat1_df.to_excel(writer, sheet_name = '시간')

		writer.save()
		result_df = pd.DataFrame(route_list)	
		result_df.to_csv("./test1/"+day+"_"+DF_element+"_result.csv",encoding='euc-kr')
		logger.debug("---------------------------------------------------------------------------------------------")
		logger.info("---------------------------------------------------------------------------------------------")
