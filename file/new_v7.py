...
def situation_update(temp_df, Max_point, kappa, trace_list, average_speed, drive_time, before_distance, day, get_up, get_off, distance_list):
	...
	if left_drive_time - DP_time <0 or left_kappa <0 :
		...
		return trace_list, left_drive_time, int(left_kappa), total_distance, route_check, distance_list
...
def route(temp_df, df_element,day, current_point):
	...
	#업데이트 현황
	trace_list, left_drive_time, left_kappa, total_distance, route_check, distance_list = situation_update(temp_df[temp_df['stopby']==0], Max_distance_row_point, left_kappa, trace_list, average_speed, left_drive_time, total_distance, day,get_up, get_off, distance_list)
	...
	while (len(temp_df.loc[temp_df['stopby']==0]) >0):
		...
		if route_check == False:
			...
			#업데이트 현황
			trace_list, left_drive_time, left_kappa, total_distance, route_check, distance_list = situation_update(temp_df, Max_distance_deg_point, left_kappa, trace_list, average_speed, left_drive_time, total_distance, day, get_up, get_off, distance_list)
...
for day in week_num:
	for DF_element in  df_DP['DP'].values.tolist():
		...
		temp_df, route_list, total_distance = route(temp_df, DF_element, day, current_point)
		...
		result_df.to_csv("./test0/"+day+"_"+DF_element+"_result.csv",encoding='euc-kr')
		logger.debug("---------------------------------------------------------------------------------------------")
		logger.info("---------------------------------------------------------------------------------------------")
