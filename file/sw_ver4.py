...
df_info = pd.read_excel('sheet_3.xlsx')
# merge the info
...
for day in week_num:
	...
	for DF_element in  df_DP['DP'].values.tolist():
		...
		#while(True):
		while(len(temp_df[temp_df['stopby']==1])>0):
			print("<><><><><><><><><><><><><><><>", count,"<><><><><><><><><><><><><><><>")
			#Rest_time_list =[]
			#Current_point = ['DP']
			#Point_list =['DP']
			...
			#get the degree
			temp_df['각도']= np.degrees(np.arctan(temp_df['DP_위도거리']/temp_df['DP_경도거리']))
			...
			### Max distance 
			Max_distance = np.max(temp_df['DP_운행거리'].values)
			## consider just the duplicated Max Value
			Max_client = temp_df.loc[temp_df['DP_운행거리']==Max_distance].iloc[0]
			#df.loc[(df['CAL_AGE']>=300),'CAL_AGE']=90
			# if the stopby the client, stopby is 1 
			temp_df.loc[(temp_df['code']==Max_client['code']),'stopby']=1
			...
			#Rest_time list
			drive_time =60*(Max_client['DP_운행거리']/Max_client['평균 운행 속도(km/h)'])
			...
			while(True):
				...
				if time - next_check_time > 0 and  Rest_Stock_list[-1]>0 :
					...
					#check the next possible time
					next_check_time = lowest_degree_row['DP_운행거리']/lowest_degree_row['평균 운행 속도(km/h)'] 
				...
				elif time - next_check_time > 0 and  Rest_Stock_list[-1] <= 0:
					...
					if Point_list[-1] == 'DP':
						...
						#again_temp_df['각도'] = np.degrees(np.arctan(again_temp_df['위도거리'] / again_temp_df['경도거리']))
						# sorted the distance 
						again_temp_df = again_temp_df.sort_values(by='DP_운행거리',ascending=True)
						# lowest distance
						#print("sdafdsfsadfasdfadsfdsafadsfsdafdsafsdafadsfasd")
						lowest_degree_row = again_temp_df.iloc[0]
					...
					Point_list.append(lowest_degree_row['code'])
					# Rest_time list
					#print("dsfadsfdsfsdffdsfasdfsafsdfadsfsdfsd")
					#print(lowest_degree_row)
					if Point_list[-2] == 'DP':
						drive_time = 60 * (lowest_degree_row['DP_운행거리'] / lowest_degree_row['평균 운행 속도(km/h)'])
						time = Rest_time_list[-1] - drive_time - lowest_degree_row['하차시간(분)'] - lowest_degree_row['상차시간(분)']
					else:
						drive_time = 60 * (lowest_degree_row['운행거리'] / lowest_degree_row['평균 운행 속도(km/h)'])
			...
			report_list.append(["No "+str(count)+"Truck ",Point_list])
			print(pd.DataFrame(report_list))
		count = count+1
		print("left :", len(temp_df[temp_df['stopby']==0]))
		#if len(temp_df[temp_df['stopby']==0])<10:
		#	break;
