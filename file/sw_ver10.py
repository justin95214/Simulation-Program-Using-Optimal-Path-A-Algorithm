	...
	for DF_element in  df_DP['DP'].values.tolist():
		...
		while(len(temp_df.loc[temp_df['stopby']==0]) >0):
			...
			if current_point =='DP':
				...
				tdf = tdf.copy()
				lowest_degree_row = tdf.sort_values(by='각도').iloc[0]
		...
		result2 = pd.DataFrame(one_truck_list,columns=["Cient Count"])
		...
		result4.to_csv(DF_element+day+"check.csv",encoding='utf-8-sig')
		#print(result)
		total_list =[]
		counting_list = []
		No_truck =1
