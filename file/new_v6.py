...
def route(temp_df, df_element,day, current_point):
	...
	#업데이트 현황
	trace_list, left_drive_time, left_kappa, total_distance, route_check = situation_update(temp_df[temp_df['stopby']==0], Max_distance_row_point, left_kappa, trace_list, average_speed, left_drive_time, total_distance, day,get_up, get_off)
	...
	while (len(temp_df.loc[temp_df['stopby']==0]) >0):
		...
		if route_check == False:
			...
			#업데이트 현황
			trace_list, left_drive_time, left_kappa, total_distance, route_check = situation_update(temp_df, Max_distance_deg_point, left_kappa, trace_list, average_speed, left_drive_time, total_distance, day, get_up, get_off)
